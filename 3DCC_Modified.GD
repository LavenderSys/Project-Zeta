...
func _process_sound(delta):
	if state == State.RUN and !footsteps.playing:
		footsteps.play()
	elif state == State.JUMP and !footsteps.playing:
		footsteps.play()
...
func _process_movement(delta):
	...
	#fall state
	if state == State.FALL:
			velocity.y += (gravity * delta * 4)/acceleration
	...
	if state > 2:
		...
		if abs(input_dir.x) < .1 && on_floor:
			...
			if movement_vector.length() < .1:
				...
				var xy = Vector2(movement_vector.x , movement_vector.z)
...
#func _process_input(delta):
	#input_dir = Vector3(Input.get_action_strength("right") - Input.get_action_strength("left"), 0 , 0)
	
#func _process_movement(delta):
#	velocity += Vector3(0,1,0) * 1.5
